...
class _MultiLayerPercep(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(input_dim, input_dim // div, bias=True),
            nn.ReLU(),            
            nn.Linear(input_dim // div, output_dim, bias=True),
        )
...
class _UserModel(nn.Module):
    ...
    def forward(self, uids, u_item_pad, u_user_pad, u_user_item_pad):
        ...
        alpha = alpha * mask_u
        #alpha = torch.tensor([1.], device=self.device).expand_as(alpha)
        ...
        alpha_s = alpha_s * mask_s
        alpha_s = torch.softmax(alpha_s, dim =2)
        #alpha_s = torch.exp(alpha_s) * mask_s
        # = alpha_s / (torch.sum(alpha_s, 2).unsqueeze(2).expand_as(alpha_s) + self.eps)
        ...
        p_i_s = mask_s.unsqueeze(2).expand_as(h_oI) * self.user_emb(uids).unsqueeze(1).expand_as(h_oI)
        beta = self.user_users_att(torch.cat([h_oI, h_oI], dim = 2).view(-1, 2 * self.emb_dim)).view(u_user_pad.size())
        mask_su = torch.where(u_user_pad > 0, torch.tensor([1.], device=self.device), torch.tensor([0.], device=self.device))
        beta = beta * mask_su
        # beta = torch.tensor([1.], device = self.device).expand_as(beta)
...
class _ItemModel(nn.Module):
    ...
    def forward(self, iids, i_user_pad):
        ...
        miu = miu * mask_i
        # miu = torch.tensor([1.], device = self.device).expand_as(miu)
        miu = torch.softmax(miu, dim = 1)
...
class GraphRec(nn.Module):
    ...
    def forward(self, uids, iids, u_item_pad, u_user_pad, u_user_item_pad, i_user_pad):
        ...
        r_ij= torch.sum(torch.mul(h_i,z_j), dim =1)
