...
def collate_fn(batch_data):
    ...
    for data, u_items_u, u_users_u, u_users_items_u, i_users_i in batch_data:
        ...
        (uid, iid, label) = data
        # if len(u_items_u) > 1 or u_items_u[0] !=0:
        #     continue
        uids.append(uid)
        iids.append(iid)
        labels.append(label)
    ...
    nuids, niids, nlabels = [], [], []
    nu_items, nu_users, nu_users_items, ni_users = [], [], [], []
    for i in range(len(uids)):
        ui = uids[i]
        for j in range(4):
            nuids.append(ui)
            nlabels.append(0)
        items = random.sample(set(iids)-set(u_items[i]),4)
        niids += items

        for k in range(4):
            nu_items.append(u_items[i])
            nu_users.append(u_users[i])
            nu_users_items.append(u_users_items[i])
        
        for item in items:
            index = iids.index(item)
            ni_users.append(i_users[index])
    uids += nuids
    iids += niids
    labels+= nlabels
    u_items += nu_items
    u_users += nu_users
    u_users_items += nu_users_items
    i_users += ni_users
